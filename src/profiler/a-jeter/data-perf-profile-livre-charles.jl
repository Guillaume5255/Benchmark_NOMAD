...
function ComputeTpsNpsMatrix(tau::Float64, runs::Array{Run_t, 1}, attr::String )
	nbSolvers, nbProblems = GetNbSolversProblems(runs)
	Tps_matrix = zeros(nbProblems,nbSolvers)
	Nps_Matrix = zeros(nbProblems,nbSolvers)
	for run in runs
		p = run.pb_num
		...
		for i in 1:size(run.eval_f)[1]
			f_acc = (run.eval_f[i]-f_x0)/(f_L-f_x0)
			if f_acc >= 1-tau

				Tps_Matrix[p,s] = 1.0
				if attr == "EVAL"
					Nps_Matrix[p,s] = run.eval_nb[i]
				end
				if attr == "ITER"
					Nps_Matrix[p,s] = run.iter[i]
				end

				if attr == "TIME"
					Nps_Matrix[p,s] = run.eval_time[i]
				end
				break
			end
		end
	end
	return Tps_matrix, Nps_matrix
end

function rpsMatrix(Nps_matrix::Array{Float64,2} )
	nbProblems, nbSolveurs = size(Tps_matrix)
	rps_matrix = zeros(nbProblems,nbSolveurs)
	for p in 1:nbProblems
		minNps = minimum(Nps_matrix[p,:])
		for s in 1:nbSolveurs
			if Tps_matrix[p,s] == 1
				rps_matrix[p,s] = Nps_matrix[p,s]/minNps
			else
				rps_matrix[p,s] = Inf
			end
		end
